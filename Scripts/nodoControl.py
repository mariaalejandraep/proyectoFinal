...
distanciaCuadricula = 0.1  #5  # .8 #1
# Numero de cuadriculas de la escena seleccionada
n = int(2.5/distanciaCuadricula)
...
def control():
    ...
    iniciar_service = rospy.Service('iniciar_recorrido', StartService, handle_start_service)
    # rospy.spin()
...
def handle_start_service(pStart, pGoal, pN_obstacles, pObstacles_array):
    global posicionActual, posicionFinal, n_obstacles, obstacles_array
    iniciarMov = True
    start = pStart
    goal = pGoal
...
def creadorVerticesCasillas():
    ...
    yInic = 2.5 - distanciaCuadricula/2
